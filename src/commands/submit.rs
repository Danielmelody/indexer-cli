...
//! indexer submit --urls https://example.com/page1 https://example.com/page2
//!
//! # Submit from file
...
//! indexer submit --sitemap https://example.com/sitemap.xml --dry-run
//!
//! # Submit to specific API only
//! indexer submit --file urls.txt --api google
//!
//! # Filter URLs with regex
//! indexer submit --sitemap https://example.com/sitemap.xml --filter "^https://example.com/blog/"
//! ```
...
pub async fn run(args: SubmitArgs, cli: &Cli) -> Result<()> {
    ...
    let indexnow_client = if should_use_indexnow(&args, &config) {
        ...
        match IndexNowClient::new(api_key, key_location, endpoints) {
            ...
            Err(e) => {
                eprintln!(
                    "  {} Failed to initialize IndexNow API: {}",
                    "✗".red(),
                    e
                );
                return Err(e);
    ...
    let db_conn = init_database(Path::new(&config.history.database_path))?;
    let history = Arc::new(HistoryManager::new(db_conn));

    let mut batch_config = BatchConfig::new()
        .with_check_history(!args.skip_history)
...
async fn collect_urls(args: &SubmitArgs, cli: &Cli) -> Result<Vec<String>> {
    ...
    if !args.urls.is_empty() {
        if !cli.quiet {
            println!("  {} Reading {} URLs from command line", "→".cyan(), args.urls.len());
        }
    ...
    if let Some(ref file_path) = args.file {
        if !cli.quiet {
            println!("  {} Reading URLs from file: {}", "→".cyan(), file_path.display());
        }
...
fn print_result_text(
    ...
    if result.is_success() {
        ...
        println!(
            "{}",
            "⚠ Some submissions failed. Check the errors above.".yellow().bold()
        );
    }
...
mod tests {
    ...
    fn test_deduplicate_urls() {
        ...
        assert_eq!(result[0], "https://example.com/page1");
        assert_eq!(result[1], "https://example.com/page2");
        assert_eq!(result[2], "https://example.com/page3");
    }
